...
function strategy(validate, options = {}) {
  ...
  const authenticate = function (req) {
    var username = lookup (req.body, _usernameField) || lookup(req.query, _usernameField);
    var password = lookup(req.body, _passwordField) || lookup(req.query, _passwordField);
